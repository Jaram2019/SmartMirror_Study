...
router.get('/', function (req, res) {
    ...
    let week_src = 'https://shuttle.jaram.net/semester/week/shuttlecock'
    request(week_src, function(err, res, body){
        if(!err && res.statusCode == 200) {
            ...
            var nearHour = find_close_hour(time, week_data);
...
function find_close_hour(now, target){// week_data.shuttleA
    ...
    for (let i = 0; i < target.length; i++) {
        ...
        if (spl_time[0] == nowTimeArr[0] || spl_time[0] == String(Number(nowTimeArr[0])+1)){
            // console.log(nowTimeArr[0]+1)
